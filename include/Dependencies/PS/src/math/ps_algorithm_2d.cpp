...
namespace ps
{
    ...
    {
        return (c.x_ <= Math::max(a.x_, b.x_) && c.x_ >= Math::min(a.x_, b.x_) &&
                c.y_ <= Math::max(a.y_, b.y_) && c.y_ >= Math::min(a.y_, b.y_));
    }
    ...
    Vector2 GeometryAlgorithm2D::lineIntersection(const Vector2 &p1, const Vector2 &p2, const Vector2 &q1,
                                                  const Vector2 &q2)
    {
        const real d = (p1.x_ - p2.x_) * (q1.y_ - q2.y_) - (p1.y_ - p2.y_) * (q1.x_ - q2.x_);
        if (realEqual(d, 0))
            return Vector2();
        const real x = ((p1.x_ * p2.y_ - p1.y_ * p2.x_) * (q1.x_ - q2.x_) - (q1.x_ * q2.y_ - q1.y_ * q2.x_) * (p1.x_ - p2.x_)) / d;
        const real y = ((p1.x_ * p2.y_ - p1.y_ * p2.x_) * (q1.y_ - q2.y_) - (q1.x_ * q2.y_ - q1.y_ * q2.x_) * (p1.y_ - p2.y_)) / d;
        return Vector2(x, y);
    }
                                                                     ...
                                                                     const Vector2 &c)
    {
        ...
        Matrix2x2 coef_mat{2.0f * (b.x_ - a.x_), 2.0f * (c.x_ - b.x_), 2.0f * (b.y_ - a.y_), 2 * (c.y_ - b.y_)};
    ...
    std::vector<Vector2> GeometryAlgorithm2D::grahamScan(const std::vector<Vector2> &vertices)
    {
        ...
        std::sort(points.begin(), points.end(), [](const Vector2 &a, const Vector2 &b)
                  {
				if (atan2l(a.y_, a.x_) != atan2l(b.y_, b.x_))
					return atan2l(a.y_, a.x_) < atan2l(b.y_, b.x_);
				return a.x_ < b.x_; });
        uint32_t targetIndex = 0;
        real targetX = points[0].x_;
        for (int i = 1; i < points.size(); ++i)
        {
            if (points[i].x_ < targetX)
            {
                targetIndex = i;
                targetX = points[i].x_;
            }
            if (realEqual(points[i].x_, targetX))
                if (points[i].y_ < points[targetIndex].y_)
                    targetIndex = i;
        }
        ...
        const real &a, const real &b, const Vector2 &p1, const Vector2 &p2)
    {
        Vector2 p_line;
        Vector2 p_ellipse;
        if (realEqual(p1.y_, p2.y_))
        {
            if (!((p1.x_ > 0 && p2.x_ > 0) || (p1.x_ < 0 && p2.x_ < 0))) // different quadrant
            {
                p_ellipse.set(0, p1.y_ > 0 ? b : -b);
                p_line.set(0, p1.y_);
            }
            else
            {
                p_line.set(std::fabs(p1.x_) > std::fabs(p2.x_) ? p2.x_ : p1.x_, p1.y_);
                p_ellipse = shortestLengthPointOfEllipse(a, b, p_line);
            }
        }
        else if (realEqual(p1.x_, p2.x_))
        {
            if (!((p1.y_ > 0 && p2.y_ > 0) || (p1.y_ < 0 && p2.y_ < 0))) // different quadrant
            {
                p_ellipse.set(p1.x_ > 0 ? a : -a, 0);
                p_line.set(p1.x_, 0);
            }
            else
            {
                p_line.set(p1.x_, std::fabs(p1.y_) > std::fabs(p2.y_) ? p2.y_ : p1.y_);
                p_ellipse = shortestLengthPointOfEllipse(a, b, p_line);
            }
        }
        else
        {
            ...
            const real k = (p2.y_ - p1.y_) / (p2.x_ - p1.x_);
            const real k2 = k * k;
            const real a2 = a * a;
            const real b2 = b * b;
            ...
            if (fuzzyIsCollinear(p1, p2, f_proj))
            {
                p_ellipse = f;
                p_line = f_proj;
            }
    ...
    std::optional<Vector2> GeometryAlgorithm2D::raycast(const Vector2 &p, const Vector2 &dir, const Vector2 &a,
                                                        const Vector2 &b)
    {
        const real denominator = (p.x_ - dir.x_) * (a.y_ - b.y_) - (p.y_ - dir.y_) * (a.x_ - b.x_);

        if (realEqual(denominator, 0))
            return std::nullopt;

        const real t = ((p.x_ - a.x_) * (a.y_ - b.y_) - (p.y_ - a.y_) * (a.x_ - b.x_)) / denominator;
        const real u = ((dir.x_ - p.x_) * (p.y_ - a.y_) - (dir.y_ - p.y_) * (p.x_ - a.x_)) / denominator;
        if (t >= 0 && u <= 1.0 && u >= 0)
            return std::optional<Vector2>({p.x_ + t * (dir.x_ - p.x_), p.y_ + t * (dir.y_ - p.y_)});
        return std::nullopt;
    }
    ...
    {
        const real xmin = topLeft.x_;
        const real ymin = bottomRight.y_;
        const real xmax = bottomRight.x_;
        const real ymax = topLeft.y_;
        real txmin, txmax, tymin, tymax;
        real txenter, txexit, tyenter, tyexit;
        real tenter, texit;
        if (realEqual(dir.x_, 0) && !realEqual(dir.y_, 0))
        {
            tymin = (ymin - p.y_) / dir.y_;
            tymax = (ymax - p.y_) / dir.y_;
            tenter = Math::min(tymin, tymax);
            texit = Math::max(tymin, tymax);
        }
        else if (!realEqual(dir.x_, 0) && realEqual(dir.y_, 0))
        {

            txmin = (xmin - p.x_) / dir.x_;
            txmax = (xmax - p.x_) / dir.x_;
    ...
    {
        return Math::isInRange(p.x_, topLeft.x_, bottomRight.x_) &&
               Math::isInRange(p.y_, bottomRight.y_, topLeft.y_);
    }
    ...
    {
        Vector2 target;
        if (realEqual(direction.x_, 0))
        {
            const int sgn = direction.y_ < 0 ? -1 : 1;
            target.set(0, sgn * b);
        }
        else if (realEqual(direction.y_, 0))
        {
            const int sgn = direction.x_ < 0 ? -1 : 1;
            target.set(sgn * a, 0);
        }
        else
        {
            const real k = direction.y_ / direction.x_;
    ...
    Vector2 GeometryAlgorithm2D::calculateCapsuleProjectionPoint(const real &halfWidth, const real &halfHeight, const Vector2 &direction)
    {
        ...
        if (halfWidth >= halfHeight) // Horizontal
        {
            ...
            const real offset = direction.x_ >= 0 ? halfWidth - radius : radius - halfWidth;
        ...
        else // Vertical
        {
            ...
            const real offset = direction.y_ >= 0 ? halfHeight - radius : radius - halfHeight;
            target = direction.normal() * radius;
            target.y_ += offset;
        }
        return target;
    ...
    {
        // https://stackoverflow.com/a/2932601_

        const real det = dir1.y_ * dir2.x_ - dir1.x_ * dir2.y_;
        // assert(!realEqual(det, 0));
        const real u = (dir2.x_ * (p2.y_ - p1.y_) - dir2.y_ * (p2.x_ - p1.x_)) / det;
        return p1 + dir1 * u;
    }
}
