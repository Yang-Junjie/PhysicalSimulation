...
namespace ps
{
        ...
        for (auto &&elem : m_contactTable)
        {
            ...
            if (m_velocityBlockSolver && elem.second.size() == 2)
            {
                ...
                for (;;)
                {
                    ...
                    if (nx.x_ >= 0.0f && nx.y_ >= 0.0f)
                        break;

                    // 2. b_1 < 0 && b_2 > 0
                    nx.x_ = vcp1.effectiveMassNormal * -b.x_;
                    nx.y_ = 0.0f;
                    jv1 = 0.0f;
                    jv2 = A.e12() * nx.x_ + b.y_;
                    if (nx.x_ >= 0.0f && jv2 >= 0.0f)
                        break;

                    // 3. b_1 > 0 && b_2 < 0
                    nx.x_ = 0.0f;
                    nx.y_ = -vcp2.effectiveMassNormal * b.y_;
                    jv1 = A.e21() * nx.y_ + b.x_;
                    jv2 = 0.0f;
                    if (nx.y_ >= 0.0f && jv1 >= 0.0f)
                        break;
                ...
                vcp1.accumulatedNormalImpulse = nx.x_;
                vcp2.accumulatedNormalImpulse = nx.y_;
            }
            else
            {
                for (auto &&ccp : elem.second)
            ...
            for (;;)
            {
                if (m_positionBlockSolver && elem.second.size() == 2)
                {
                    ...
                    for (;;)
                    {
                        ...
                        x.x_ = -b.x_ / k11;
                        x.y_ = 0.0f;
                        bias2 = A.e21() * x.x_ + b.y_;
                        if (x.x_ >= 0.0f && bias2 >= 0.0f)
                        {
                            d = x;
                            break;
                        }
                        ...
                        x.x_ = 0.0f;
                        x.y_ = -b.y_ / k22;
                        bias1 = A.e12() * x.y_ + b.x_;
                        if (x.y_ >= 0.0f && bias1 >= 0.0f)
                        {
                            d = x;
                            break;
                        }
                    ...
                    Vector2 impulse1 = vcp1.normal * d.x_;
                    Vector2 impulse2 = vcp2.normal * d.y_;
    ...
    void ContactMaintainer::add(const Collision &collision)
    {
        ...
        const bool isRoundA = bodyA->shape()->type() == ShapeType::Circle;
        const bool isRoundB = bodyB->shape()->type() == ShapeType::Circle;
        ...
        for (auto &&iter : m_contactTable)
        {
            ...
            contactList.erase(
                std::remove_if(contactList.begin(), contactList.end(),
                               [](const ContactConstraintPoint &ccp)
                               { return !ccp.active; }),
                contactList.end());
        }
        for (auto it = m_contactTable.begin(); it != m_contactTable.end();)
        {
            if (it->second.empty())
                it = m_contactTable.erase(it);
            else
                ++it;
        }
    }
