...
namespace ps
{
	...
	void Narrowphase::reconstructSimplexByVoronoi(Simplex& simplex)
	{
		 ...
		 * solve for u,v,w
		 */
		const real det = a.y_ * b.x_ - a.x_ * b.y_ + a.x_ * c.y_ - a.y_ * c.x_ + b.y_ * c.x_ - c.y_ * b.x_;
		assert(det != 0.0f);
		const real u = (b.y_ * c.x_ - c.y_ * b.x_) / det;
		const real v = (c.y_ * a.x_ - a.y_ * c.x_) / det;
		const real w = 1 - u - v;
	                                 ...
	                                 const Vector2& dir)
	{
		...
		{
			direction.set(-direction.y_ + static_cast<real>(i), -direction.x_ - static_cast<real>(i));
			SimplexVertex v = support(shapeA, shapeB, direction);
			simplex.vertices[0] = v;
			direction.set(-direction.y_ - static_cast<real>(i) - 0.5f, -direction.x_ + static_cast<real>(i) + 0.5f);
